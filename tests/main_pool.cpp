...
public:
    ...
    TestObject(const char *value) { std::cout << "TestObject constructor with value [" << value << "]" << std::endl;}
...
int testPool() {
    ...
    // Pool<TestObject>::Object obj1 = myPool.acquire("PTDR");
    ...
    //
    // // obj1 and obj3are released back to the pool when the program ends
    // return 0;
	Pool<std::string> pool;

	auto tmp = pool.acquire("hello");
	std::cout << tmp._obj << std::endl;
    return 0;
}
